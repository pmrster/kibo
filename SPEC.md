...
- **Fonts**: The app's name is set in **Space Grotesk**. This is the one non-system face, and it is **not currently bundled** — it resolves only where the font happens to be installed, and falls back to the system font's condensed black everywhere else. Bundling it is required before distributing a build that should look as designed. Other English chrome uses the plain system font so the app looks native. Thai content — the input, the result, examples, and the privacy line — uses **Noto Sans Thai**, which macOS ships: the system's Thai face crowds vowel and tone marks at these sizes, and reading those marks is precisely what this app asks of people. Where the family is unavailable, every call falls back to the system font.
