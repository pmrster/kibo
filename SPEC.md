...
- **Fonts**: **Prefer system fonts, and bundle none** — a face the app cannot guarantee is a face it should not depend on, since the shipped build would silently fall back and look nothing like the approved design. The app's name uses the system font at heavy weight, the same face as the section labels. Space Grotesk was tried and dropped for exactly this reason: it looked right, but it was installed on one machine only. Other English chrome uses the plain system font so the app looks native. Thai content — the input, the result, examples, and the privacy line — uses **Noto Sans Thai**, which macOS ships: the system's Thai face crowds vowel and tone marks at these sizes, and reading those marks is precisely what this app asks of people. Where the family is unavailable, every call falls back to the system font.
