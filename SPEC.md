...
- **Fonts**: Prefer system fonts, and bundle none. The app's name is set in the system font's **condensed black** weight — a utility, not a toy, and the narrower letters carry a five-word title without shrinking it. Other English chrome uses the plain system font so the app looks native. Thai content — the input, the result, examples, and the privacy line — uses **Noto Sans Thai**, which macOS ships: the system's Thai face crowds vowel and tone marks at these sizes, and reading those marks is precisely what this app asks of people. Where the family is unavailable, every call falls back to the system font.
...
- **Mascot**: An original 8-bit ghost — the wrong layout haunting your sentence, and a peekaboo joke the converter can act out. It hides behind the input field with only its eyes over the edge, rises fully when there is a result, and shuts its eyes contentedly after a copy. It appears in the converter, About, and Settings. The peekaboo is done by occlusion rather than by the ghost covering its own face: at this resolution an outlined shape over a face reads as spectacles, not as hands.
- **Mascot colours**: Body and outline swap between appearances — midnight on the light panel, pale on the dark one — so it is always the opposite of what it sits on. A single warm yellow marks the mouth, and is used nowhere else.
- **Menu-bar glyph**: A keycap outline with `ก` on it — deliberately *not* the mascot. A glyph up there has to identify the app, and matching a sibling app defeats that; an early cat silhouette was indistinguishable from Tama's at 18px. The outline is the distinguishing feature: nothing else in a menu bar has a border, and it depicts the key the user forgot to press.
